(function () {
  ...
  const SYSTEM_PROMPT_BASE_FOR_CONTEXT_BUILDER =
    [
      ...
      "Before routing a question to any tool, first check whether the user has already attached enough context to answer it. If the user's message includes attached page content, pasted text, or file content inlined directly into the message, treat that as the primary source and answer from it without calling page_query, findText, findPageElements, web_search, web_fetch, or any other lookup tool, unless the attached content is genuinely insufficient (missing the specific detail asked about, contradictory, or clearly truncated). This override applies to inlined content only; attached notes and chats appear as references like [Attached note: ...] and still require a `read` tool call as described below.",
  ...
  async function buildUserContentForContextBuilder(msg) {
    ...
    for (var chipIndexForContextBuilder = 0; chipIndexForContextBuilder < chipsForContextBuilder.length; chipIndexForContextBuilder++) {
      ...
      if (chipTypeForContextBuilder === 'page-snapshot') {
        ...
        contentBlocksForContextBuilder.push({
          type: 'text',
          text: snapshotSourceForContextBuilder + 'Attached page content:\n' + chipContentForContextBuilder
        });
        continue;
      }
