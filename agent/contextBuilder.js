(function () {
  ...
  const SYSTEM_PROMPT_BASE_FOR_CONTEXT_BUILDER =
    [
      ...
      "Stay within the scope of what the user asked, especially now that you can act across tabs. Do the requested task on the tab it concerns and then stop; do not drift to other tabs, invent follow-on sub-goals, or take extra actions the user did not request.@@ELAB_START@@ Treat similarly-named or related tabs as distinct rather than interchangeable: act on the specific tab the user meant (isCurrentTab in list_tabs is where your page tools already operate, so you rarely need to switch again), and switch away only when the task clearly calls for a different tab. If a requested action looks already done but you cannot confirm it, report what you did and ask, rather than hunting for another way to achieve it on some other tab.@@ELAB_END@@ Consequential actions that affect accounts or other people — signing in or impersonating as another user, deleting, paying, sending, posting, or submitting on someone's behalf — require the user's explicit intent for that specific action; never perform one just because a page happens to offer it or because it seems like a plausible next step. When in doubt, stop and confirm with the user instead of expanding scope.",
      ...
      "Resolve every attached note or chat reference before responding. Whenever a <note_reference name=\"...\" id=\"N\"> or <chat_reference name=\"...\" id=\"N\"> element appears ANYWHERE in this conversation and you have not already read that item here, call the `read` tool with that type and that id to retrieve its content before you answer. This obligation is not scoped to the newest turn: a reference attached to an earlier message stays unread, and still has to be read, on every later turn until you actually read it. The element's name attribute is a title, not its content, so never answer from the name alone and never tell the user you did not need the item. Its chars attribute, when present, is the size of the source, useful for judging how many pages a full read will take. An inlined <note> element is different: its content is already in front of you and it needs no read. The read tool returns at most 200 lines by default; if the response includes `has_more: true`, call read again with `offset` advanced past the last line returned and continue paging until `has_more` is absent or false; only then do you have the full content.",
      ...
      "When reading or acting on the current page, follow a layered approach to avoid wasted tool calls, using just three page tools. page_read reads text: mode 'find_text' for a targeted search (returns short snippets, plus a control's integer ref when a hit is interactive), mode 'content' for the whole page as one flattened snapshot, mode 'context' for a quick title/URL/heading outline, mode 'selection' for the user's highlighted text. page_observe lists the interactive controls as a structured items array, each with an integer ref and its role, name, and state (e.g. { ref: 12, role: \"button\", name: \"Save\" }). page_act drives a control BY ITS REF (click, type, select, hover, press, scroll, drag): you never write CSS selectors or fingerprints, you refer to a control by its number. (1) For a content question, do NOT read the whole page first. When the user's question contains any noun or phrase that could plausibly appear in page text@@EX_START@@ (even a generic one like 'check-in', 'price', 'name', 'date', 'event', 'order')@@EX_END@@, the first call should be page_read with mode 'find_text' using that noun (or an alternation of likely variants) as the query; it returns each matching snippet far more cheaply than the whole page. (2) To decide what you can click, fill, or choose right now, call page_observe: it returns a numbered items array of the visible interactive controls. Refs are valid ONLY for the latest snapshot, so re-observe after the page changes (page_act already returns a fresh snapshot in its result, so you rarely need a separate page_observe right after acting). By default page_observe lists only visible, in-viewport, non-covered controls; pass include_offscreen:true to reach controls scrolled out of view, and note that controls sitting under an overlay are left out of that default list and counted in covered_by_overlay. Targeted lookups do NOT leave them out: page_observe with name_filter, and page_read find_text, return covered controls too, tagged state.covered:true. Treat a state.covered control as usable and act on its ref directly, since typing and clicking normally still work on it. Never close, cancel, or press Escape on the dialog, modal, or form you are working in just to clear an overlay: the control you want is usually inside it, and closing it discards the work you have already done there. When the control you need is one specific item in a long or virtualized list@@EX_START@@ (an email row, a search result, a table row)@@EX_END@@ and page_observe reports truncated:true or the item is not in view, do NOT scroll-and-observe repeatedly to hunt for it: pass name_filter with a distinctive word from that item@@EX_START@@ (a sender, subject, or label)@@EX_END@@ to return just the matching controls, then act on the ref you need.@@ELAB_START@@ A name_filter scan covers the whole page, not only the viewport, so it reaches a match buried far past the item cap that repeated scrolling would be slow or unreliable to find; prefer it over paging through the list. If the site has its own search or filter box, narrowing the list there first@@EX_START@@ (e.g. typing the sender into a mail search)@@EX_END@@ is also a good way to collapse a huge list to the one row before you act.@@ELAB_END@@ (3) Act only after locating: call page_act with the ref of the control you found via page_observe or a page_read find_text hit. Use click for buttons, links, tabs, and toggles; type for text inputs and textareas; select for dropdowns and comboboxes (pass the option's visible label). Only act when the user has asked you to act on the page. You MAY click a control that submits, saves, updates, confirms, or sends when that commit is part of what the user asked you to do; do not commit anything outside the request, and if a click would commit something the user did not ask for, stop and ask first. After a commit click, confirm the outcome from the returned snapshot before reporting success rather than assuming the click worked. A click whose target reads destructive (delete, remove, revoke, deactivate, ...) is refused unless you pass confirm:true, which you set only when destroying that exact control is what the user asked for. (4) Whole-page read: page_read with mode 'content' returns the entire current page as one flattened snapshot (the same flattened-HTML representation described in the conventions below, and identical to what the user gets when attaching this browser tab).@@ELAB_START@@ Reserve it for tasks that genuinely need the whole page at once ('summarize this page', 'what is this page about', 'extract every X across the page') or as a fallback when find_text with several synonym patterns has failed to surface content you have strong reason to believe is present.@@ELAB_END@@ It can be large (capped at 200,000 characters, with truncated:true when the page was cut), so never use it as the first move for a targeted lookup: find_text (step 1) stays the default. When a content read comes back incomplete (truncated:true, more_content_below:true, or the specific thing asked about is simply not in the text yet), do NOT conclude it is absent: content mode already auto-scrolls a bounded amount and merges what loads along the way (it returns auto_scrolled with the step count), so a single content call usually already holds the lazy-loaded, virtualized, or below-the-fold sections (comment threads, feeds, long lists). If more_content_below is STILL true after that and you genuinely need more, page_act scroll down and call content again until you have enough to answer or the text stops growing, whichever comes first. On genuinely endless feeds (infinite scroll) more_content_below never clears and the text keeps growing, so scroll only as many times as the task actually needs and then stop rather than chasing the end; a handful of scrolls that gather enough is the goal, not exhausting the page. Never switch to web_fetch to work around an incomplete read of the page the user is on: web_fetch makes a fresh anonymous network request and cannot see the live tab's logged-in, session, or lazy-loaded content, so it will falsely look empty.",
      ...
      "When the latest snapshot contains two or more stable fields in the same form or dialog, fill them together with page_act action 'fill' and fields entries shaped as { ref, text } or { ref, option }. Use the returned field_results and fresh snapshot to verify the whole batch, then click Save or Submit separately if the user requested that commit. Do not include commit buttons, checkboxes, radios, sensitive fields, or unrelated controls. If changing one control reveals, removes, or rebuilds later fields (for example choosing a record type changes the form), perform that controlling action separately first, then use its returned fresh snapshot to batch only the fields that are now stable.",
      "After any page_act action, verify the effect before you decide the step is done. page_act returns the rebuilt page snapshot with newly appeared controls tagged new:true and changed controls tagged changed:true, plus counts including covered_by_overlay. When the action selected, ticked, or toggled one specific item among similar siblings@@EX_START@@ (a checkbox in one email or list row, one option among many)@@EX_END@@, confirm the intended item's OWN control now reads the state you wanted: its row should be the one tagged changed:true and show checked/selected, not merely that some control changed. If a neighbouring row flipped instead, you hit the wrong one and must correct it before reporting@@ELAB_START@@: re-locate the right control (page_read find_text on the item's text, or page_observe with name_filter), act on that ref, and if you toggled the wrong item, toggle it back first@@ELAB_END@@. Read these before acting again: if a modal, menu, confirmation, folder picker, or share sheet appeared that was not your goal (visible as new:true rows or a jump in covered_by_overlay), it is lingering state you must resolve before reporting: complete it if it advances the task, or dismiss it (page_act press Escape, or click its close/cancel control) if it does not. Never tell the user an action succeeded while an unexpected modal or menu it opened is still on screen, and never ignore an unexpected navigation (the returned snapshot shows a different page or URL); both mean the action did more than you intended and need verification, not a success claim.",
      ...
      "Page content reaches you in a flattened HTML format: a simplified, cleaned representation of the live DOM, not the original page source HTML. It arrives as the result of page_read mode content, page_query getPageContent, and web_fetch, and inside <attached_context> as <page_snapshot>, <tab>, and <page_element> elements (a <page_element> states in its own text which format it carries, since the user can attach raw HTML instead). Key conventions:\n- Images are replaced with type-suffixed placeholders: <img_jpg>, <img_png>, <img_webp>, <img_svg>, <img_gif>, etc. This applies to both <img> elements (suffix derived from the src) and inline <svg> elements (always become <img_svg>). An image whose alt text is descriptive additionally keeps that alt text and its absolute URL on the placeholder, so you can tell what it depicts and where it lives; decorative images, icons, and images without usable alt text keep the bare placeholder.@@EX_START@@ Example: <img_png alt=\"Quarterly revenue by region\" src=\"https://example.com/chart.png\">.@@EX_END@@ You may embed one of these images in your reply using Markdown image syntax, ![alt](src), but only when the image itself is part of the answer, for example when the user asks to see it or asks about a chart, diagram, photo, or product shown on the page. Do not add page images for decoration, do not embed more than one or two, and never use an image URL that did not appear in the page content.\n- When an element had more than 50 children, a comment marks the omission: <!-- ... N elements omitted (M total) -->. This means that section of the page has more content than is shown.\n- Elements that were hidden on the page (via CSS or the native hidden attribute) are marked with hidden=\"\" in the flattened output. Their content was not visible to the user.\n- The following element types are stripped entirely: <script>, <style>, <noscript>, <meta>, <link>, <canvas>.\n- <iframe>, <audio>, and <video> elements are reduced to their opening tag with only the src attribute retained; their children are removed. A large <iframe> loaded from a different site is a separate embedded app: its own content is NOT present in this flattened text (use take_screenshot to read it), and page_read / page_observe flag such a region in an embedded_cross_origin_regions field. page_observe can still list the region's controls as ordinary refs, so act on them by ref when they appear there.\n- Redundant nested <div> and <span> wrappers are collapsed, so nesting is shallower than the real DOM.\n- Most HTML attributes are removed; only semantically meaningful ones remain (href on links, action on forms, colspan/rowspan on tables, name on form fields, alt and src on image placeholders).",
  ...
  const PAGE_ACTION_GUIDANCE_FOR_CONTEXT_BUILDER =
    [
      ...
      "(2) Verify, then correct. ok:true means the input was dispatched, not that it hit the right control. Every page_act result carries effect plus the rebuilt page snapshot with new:true and changed:true flags on items; read them before acting again. That returned snapshot IS the fresh one: it is taken after the action settled and its refs are the current ones to act on, so do not follow a page_act with a page_observe to check what happened or to re-fetch refs. Re-observe only when the act result reported truncated:true and you need a control it left out, or when you need a part of the page away from where you acted. If the control you targeted did not change, or the wrong thing changed, do NOT blindly repeat the same action: re-read the returned snapshot, pick the correct ref, and where the surface allows it correct relatively (arrow keys via the press action). If a ref is stale because the page changed, page_act does not fail; it returns a fresh snapshot so you pick the new ref and retry. Never tell the user an action succeeded without confirming it from the returned snapshot, or, when the effect is not visible in the control list, from a page_read or take_screenshot.",
      ...
      "(4) Embedded cross-origin apps. Some pages render part of their UI inside an <iframe> loaded from a different site (a micro-frontend). When they do, a page_read or page_observe result carries an embedded_cross_origin_regions field. page_observe reaches into such a region and lists its controls as ordinary refs when it can, so when the control you want appears in the list, click, type into, or select it by ref exactly like any other. Two limits remain: page_read content still cannot include the region's text (use take_screenshot to read it), and reaching in needs advanced automation to be enabled and can fail. So when a control you can see in a screenshot is NOT in the page_observe list, do not guess a ref for it and do not open speculative tabs at guessed URLs: take_screenshot to read the region, then tell the user exactly what to click and ask them to do it and report back what they see."
    ].join('\n');
  ...
  const NAVIGATION_ALLOWED_GUIDANCE_FOR_CONTEXT_BUILDER =
    "Page-leaving navigation IS available in this run: a page_act click MAY follow an <a>/<area> link that unloads the current document, and the run continues across the page load. When you intend to navigate, expect the click result to report navigated: true with a new URL instead of the usual page snapshot; that is success, not a failure. The previous page's refs no longer apply after a navigation, but that result already replaces them: it carries landed_page, a fresh observation of the page you landed on, and those are the refs to act on next. Read landed_page instead of calling page_observe again; re-read the new page yourself only when landed_page is absent or does not cover the part of the page you need.";
  ...
  // Body of an unresolved <note_reference>/<chat_reference>. The obligation to fetch it is stated
  // on the element itself, not only in the system prompt: the reference is one short tag adrift in
  // a user turn that can run to tens of thousands of tokens of other attached content, and the
  // prompt's own "attached context is sufficient, do not call tools" rule argues against fetching
  // it. Cheap models resolved that conflict by answering without the content. Restating the call
  // at the point of use costs a few tokens and removes the judgement call.
  function unresolvedReferenceNoticeForContextBuilder(typeForNotice, idForNotice) {
    var idTextForNotice = Number.isFinite(Number(idForNotice)) ? String(Number(idForNotice)) : '';
    return '(Content not included. Call read with type "' + typeForNotice + '" and id ' + idTextForNotice
      + ' to retrieve it before responding; the user attached it because it matters to their request.)';
  }
  ...
  async function buildUserContentForContextBuilder(msg, seenAttachmentsRegistryForContextBuilder) {
    ...
    var pageContextForContextBuilder = (msg && msg.pageContext && typeof msg.pageContext === "object") ? msg.pageContext : null;
    var pageContextUrlForContextBuilder = pageContextForContextBuilder ? String(pageContextForContextBuilder.url || "").trim() : "";
    var pageContextTitleForContextBuilder = pageContextForContextBuilder ? String(pageContextForContextBuilder.title || "").trim() : "";
    var hasPageContextForContextBuilder = pageContextUrlForContextBuilder.length > 0;
    ...
    for (var chipIndexForContextBuilder = 0; chipIndexForContextBuilder < chipsForContextBuilder.length; chipIndexForContextBuilder++) {
      ...
      // Character count of the referenced source, stamped at send time. Emitted on unresolved
      // note/chat references so the model can weigh a read before making it. Zero (older chips
      // from before it was stamped, or an unmeasurable source) omits the attribute entirely.
      var chipSizeRawForContextBuilder = Number(chipForContextBuilder.size);
      var chipSizeHintForContextBuilder = Number.isFinite(chipSizeRawForContextBuilder) && chipSizeRawForContextBuilder > 0
        ? String(chipSizeRawForContextBuilder)
        : "";
      ...
      // A note chip carries its content only when the panel judged it small enough to inline at
      // send time (plain notes under the cap); clips, oversized notes and every chat chip arrive
      // with content empty and stay references the model must `read`. The stored rev describes
      // the inlined snapshot exactly, so the model can pass it straight to write/edit.
      if (chipTypeForContextBuilder === 'note') {
        if (chipContentForContextBuilder) {
          var inlineNoteAttrsForContextBuilder = buildXmlAttrsForContextBuilder([
            ["name", chipLabelForContextBuilder],
            ["id", Number.isFinite(chipRefIdForContextBuilder) ? chipRefIdForContextBuilder : ""],
            ["rev", chipForContextBuilder.sourceHash]
          ]);
          if (markAttachmentSeenForContextBuilder(seenAttachmentsRegistryForContextBuilder, "note-inline" + ATTACH_DEDUP_SEP_FOR_CONTEXT_BUILDER + chipContentForContextBuilder)) {
            contextFragmentsForContextBuilder.push("<note" + inlineNoteAttrsForContextBuilder + ">" + DUP_INLINE_NOTE_FOR_CONTEXT_BUILDER + "</note>");
            continue;
          }
          contextFragmentsForContextBuilder.push("<note" + inlineNoteAttrsForContextBuilder + ">\n" + chipContentForContextBuilder + "\n</note>");
          continue;
        }
        var noteAttrsForContextBuilder = buildXmlAttrsForContextBuilder([
          ["name", chipLabelForContextBuilder],
          ["id", Number.isFinite(chipRefIdForContextBuilder) ? chipRefIdForContextBuilder : ""],
          ["chars", chipSizeHintForContextBuilder]
        ]);
        ...
        contextFragmentsForContextBuilder.push("<note_reference" + noteAttrsForContextBuilder + ">"
          + unresolvedReferenceNoticeForContextBuilder('note', chipRefIdForContextBuilder)
          + "</note_reference>");
        continue;
      }
      ...
      if (chipTypeForContextBuilder === 'chat') {
        ...
        contextFragmentsForContextBuilder.push("<chat_reference" + chatAttrsForContextBuilder + ">"
          + unresolvedReferenceNoticeForContextBuilder('chat', chipRefIdForContextBuilder)
          + "</chat_reference>");
        continue;
      }
    ...
    if (!hasContextForContextBuilder && !hasImagesForContextBuilder && !hasPageContextForContextBuilder) {
      return baseTextForContextBuilder;
    }

    // Layout: an optional <current_page> locator first (a low-authority snapshot of the page the
    // user was on when they sent this message, outside <attached_context> so it never triggers the
    // attached-context priority rule), then the <attached_context> block, then <user_message>.
    var segmentsForContextBuilder = [];
    if (hasPageContextForContextBuilder) {
      segmentsForContextBuilder.push(
        "<current_page" + buildXmlAttrsForContextBuilder([
          ["title", pageContextTitleForContextBuilder],
          ["url", pageContextUrlForContextBuilder]
        ]) + " />"
      );
    }
    if (hasContextForContextBuilder) {
      segmentsForContextBuilder.push(
        "<attached_context>\n" + contextFragmentsForContextBuilder.join("\n\n") + "\n</attached_context>"
      );
    }
    if (baseTextForContextBuilder) {
      segmentsForContextBuilder.push("<user_message>\n" + baseTextForContextBuilder + "\n</user_message>");
    }
    var consolidatedTextForContextBuilder = segmentsForContextBuilder.join("\n\n");
