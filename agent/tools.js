(function () {
  ...
  ns.toolDefs = [
...

    {
      ...
      function: {
        name: 'page_observe',
        description: 'List the interactive controls on the page, each tagged with a stable integer ref you hand to page_act to act on it. This is the PRIMARY way to see and drive a page: you refer to a control by its number, never by a CSS selector. Returns a structured items array ([{ ref, role, name?, value?, placeholder?, type?, state? }]), a snapshotId, page { title, url }, returned/truncated flags, and counts { total_interactive, visible, in_viewport, covered_by_overlay }. Only visible, in-viewport, non-covered controls are listed by default; controls hidden behind an overlay are omitted and reported in covered_by_overlay (dismiss the overlay to reach them). Refs are valid ONLY for the latest snapshot: after anything changes the page, re-observe. (page_act already returns a fresh snapshot in its result, so you rarely call page_observe twice in a row.) Read-only DOM scan; it does not attach the debugger.',
        parameters: {
          type: 'object',
          properties: {
...

    {
      ...
      function: {
        name: 'page_read',
        description: 'Read page content without selectors. Pick a mode: "selection" returns the user\'s current text selection ({ selected, text }); "context" returns the page title/url plus a compact indented outline of the visible headings, for quick orientation; "content" returns the main readable text of the page (capped at 200,000 chars, with truncated: true when cut). content already captures everything in the DOM, including below-the-fold nodes, but pages that lazy-load, infinite-scroll, or virtualize their lists only put more content into the DOM as you scroll toward it. content therefore also returns more_content_below (with more_content_reason of "content-grew", "virtualized-list", "loading-indicator", or "lazy-media") to tell you whether more is likely still to load: when it is true and you need the rest, page_act scroll down, then call content again, repeating until it goes false or the text stops growing. "content-grew" means this read was longer than your previous read of the same page, i.e. your last scroll actually loaded more, so keep going. IMPORTANT: more_content_below: false is not a hard guarantee the page is fully loaded (a batch may not reveal its next loader until you scroll further); if the text clearly grew or changed since your last read, scroll and read once more to confirm. For a "virtualized-list" the visible rows swap in and out as you scroll rather than accumulating, so scroll in SMALL steps (pass a modest amount to page_act) and combine the reads you take along the way, and note the flag cannot detect the end of a virtualized list. For genuinely endless feeds the flag never clears, so scroll only enough times to answer the task, then stop. "find_text" does a literal, case-insensitive substring search over visible text and returns each hit as a snippet, and crucially, when a hit lands on (or inside) an interactive control it carries that control\'s integer ref so you can act on it directly with page_act. find_text returns { query, snapshotId, count, actionable, matches:[{ snippet, ref?, role?, name? }], text } where text lists actionable hits as `[9] link "Study Group" — …snippet…` and informational hits as `(text) …snippet…`. Use find_text to locate a labeled control or a phrase and jump straight to acting on it; use content/context to understand the page. Read-only; no debugger.',
        parameters: {
          type: 'object',
          properties: {
            mode: { type: 'string', enum: ['selection', 'context', 'content', 'find_text'], description: 'Which read to perform.' },
            query: { type: 'string', description: 'For mode "find_text": the literal text to search for (case-insensitive by default). Required for find_text.' },
