(function () {
  ...
  ns.toolDefs = [
...

    {
      ...
      function: {
        name: 'page_observe',
        description: 'List the interactive controls on the page, each tagged with a stable integer ref you hand to page_act to act on it. This is the PRIMARY way to see and drive a page: you refer to a control by its number, never by a CSS selector. Returns a structured items array ([{ ref, role, name?, value?, placeholder?, type?, state? }]), a snapshotId, page { title, url }, returned/truncated flags, and counts { total_interactive, visible, in_viewport, covered_by_overlay }. state carries flags such as checked/expanded/pressed/selected/disabled/offscreen, state.hasPopup on a control that opens a popup (a dropdown/menu trigger you open before its options exist), and, on the one control holding keyboard focus, state.focused:true. A keyboard action (press, type) lands on the focused element, so check which ref has state.focused before pressing a key: if it is not the control you mean to drive, click that control (or press with its ref, which focuses it first) so the key is not sent to the wrong place. Only visible, in-viewport, non-covered controls are listed by default; other covered controls are counted in covered_by_overlay but omitted from the default list to avoid flooding it with modal-backdrop noise. Exception: when a dialog/modal is open, its OWN controls are always listed even if they read as covered (a just-opened modal is briefly hit-tested as covered), so a modal you just opened returns its fields directly without a name_filter. When you pass name_filter, matching controls are returned even if currently covered, each tagged state.covered:true: synthetic page_act (click/type) usually still works on those refs, so act on them directly rather than assuming you must scroll or dismiss an overlay first. "|" in name_filter separates alternatives (same as find_text). A filtered scan covers the whole page, not just the viewport. A name_filter that matches nothing returns items: [] with a note; if every returned match is covered, a short note explains state.covered without blocking action. A given control keeps the same ref number across re-observes as long as it stays on the page, but always act only on refs from your LATEST snapshot, whichever tool produced it. Needing a fresh snapshot after the page changes does NOT always mean calling this tool: page_act already returns one covering the area it just acted on, taken after the action settled, and its refs are exactly as valid as this tool\'s. So after a page_act, use the snapshot it returned; call page_observe again only when that result reported truncated:true and you need a control it left out, or when you need a part of the page away from where you acted. Read-only DOM scan; it does not attach the debugger.',
        parameters: {
          type: 'object',
          properties: {
...

    {
      ...
      function: {
        name: 'page_act',
        description: 'Act on a page control by its integer ref from your latest page snapshot: no selectors, no fingerprints. A ref is valid when it came from the most recent snapshot you were handed, whichever tool produced it: page_observe, a page_read find_text hit (including a match\'s controls list), or the post-action snapshot a previous page_act returned. Those three are equivalent sources, and the snapshot page_act hands back is a first-class one, so act on its refs directly rather than calling page_observe again to re-derive them. Never guess, offset, or reuse a ref from an older result. A ref that was not in the latest result is rejected with a fresh snapshot to pick from: a control keeps its ref number for as long as it stays on the page, but a number you were never shown may never have been assigned, or may belong to an element that has since been replaced. Pass action plus ref for single-control actions; fill uses fields instead of top-level ref. Actions: "click" (synthetic DOM click first; if it has no effect and the target looks like a custom widget, escalate to trusted CDP only when a fresh post-scroll hit-test shows the control is reachable; if still covered, trusted is skipped and the error explains both legs; pass button:"right" for a right-click); "type" (synthetic value-set first; same trusted gate before focus+keystrokes); "select" (synthetic open/pick first; on failure escalates to a trusted click and then to a focus + ArrowDown keyboard open, always committing the exact matched option; if no path reveals the option list the error tells you to open it manually with click then press); "fill" (fill up to 10 related, non-sensitive fields from one current snapshot using fields:[{ref,text}|{ref,option}]; use this for two or more stable fields in one form/dialog, never for Save/Submit or controls whose change reveals or rebuilds the remaining fields); "hover"; "press" (send a key or chord via keys, e.g. "Enter", "Escape", "ArrowDown", "Ctrl+A"; pass ref to move focus to that control first so the key lands on it, or omit ref / pass 0 to send to whatever already holds focus; if a passed ref cannot take focus the key is NOT sent and the error says to click it first); "scroll" (scroll ref into view, OR omit ref / pass ref:0 and direction up/down/left/right, optionally amount in pixels); "drag" (from ref to to_ref). state.covered on a ref does NOT block synthetic click/type: try the action; only trusted coordinate input cares about occlusion. If a ref is stale because the page changed, the call does NOT fail: it returns a FRESH snapshot so you pick the new ref and retry. After every action it returns the rebuilt snapshot items with changed rows tagged changed:true and newly appeared rows tagged new:true, so you never diff two lists yourself; check effect and those flags to confirm what happened before acting again. This post-action snapshot is CENTERED on the control you just acted on (plus the controls that changed as a result, such as a toolbar that appears on selection), so the acted control and its new state are included even when it sits far down a long list: read it to verify the result inline. Do NOT follow a page_act with a page_observe to see what happened or to refresh your refs. This snapshot already IS that refresh: it is taken after the action settled, its refs are the current ones, and a duplicate observe re-sends controls you were just given at real token cost. Call page_observe after an action only when this result reports truncated:true and you need a control it left out, when you need a part of the page away from where you acted, or when something other than this action changed the page afterwards. If an action needs advanced automation and it is off, a permission prompt opens and the SAME action continues automatically once you approve, so do not abandon the step. A click whose target label reads destructive (delete, remove, revoke, deactivate, ...) is refused unless you pass confirm: true. Result: { ok, action, effect, field_results?, partial?, snapshotId, page, counts, returned, truncated, acted, dialogs?, items }. Read these before deciding the step is done, because effect only reports that the page reacted, never that the intended change committed: (a) truncated says whether this window is the whole story, measured on the same basis page_observe uses, so the two are directly comparable. truncated:false, the common case, means nothing eligible was cut and the snapshot is complete as it stands: trust it and do not re-observe. Only when truncated:true is a named field missing from items NOT evidence that it is gone or empty, and only then should you look it up with page_observe name_filter or page_read find_text instead of assuming; (b) acted:{ ref, still_connected } says what became of the control you acted on, and still_connected:false after clicking a commit control (Save, Done, Apply) usually means that control and its form were torn down, i.e. the commit went through; (c) dialogs is present only when a dialog or menu was open before or after the action, and reports { before, after, closed?, still_open?, opened? } by count and name. dialogs.closed listing the modal you were filling in is strong evidence the commit succeeded, even when the URL is unchanged (normal for modal saves, so do not read a same URL as failure). dialogs.still_open means the form is likely rejecting the input (a validation error), so stay in it and fix the field rather than reporting success. dialogs.opened means the action raised something new, such as a confirmation prompt, which you must resolve before the task is done. When the changed field already shows your new value (via changed:true in this snapshot) and the edit modal closed on Save, the edit is committed: stop there, do not re-verify. If you do need to confirm a field edit separately, re-read that field\'s value (page_observe name_filter on the field, or reopen the edit surface) and compare it to what you set; do NOT verify by page_read find_text for the value you typed, because find_text scans visible text only (never a link href or an input value) and pages routinely display a stored value in reformatted form (a URL shown scheme-stripped as "github.com/you", a truncated or masked string), so the literal you typed will not appear even when the save succeeded. If the action navigated the page (e.g. a form submit or link), the result instead has navigated:true, the new url, and landed_page: a fresh page_observe of the page you landed on (its items/refs are the current ones to act on next). Treat navigated:true as success, not failure: read landed_page to confirm the outcome before your next action, and do not repeat the action or go looking on another tab.',
        parameters: {
          type: 'object',
          properties: {
            action: { type: 'string', enum: ['click', 'type', 'select', 'fill', 'hover', 'press', 'scroll', 'drag'], description: 'The action to perform on the ref, or "fill" to fill several related fields from the same current snapshot.' },
            ref: { type: 'integer', description: 'Integer ref of the target control from the most recent page_observe / page_read find_text snapshot. Required for click, type, select, hover, and drag; optional for press (with a ref, focus is moved to that control before the key is sent; omit or pass 0 to send the key to whatever already holds focus) and scroll (omit, or pass 0, to scroll the whole page with direction).' },
            ...
            fields: {
              type: 'array',
              maxItems: 10,
              description: 'For action "fill": 1-10 related, non-sensitive fields from the latest snapshot. Each entry must have ref plus exactly one of text (a text field) or option (a dropdown): OMIT the key you are not using rather than sending it as an empty string, e.g. {"ref":10,"text":"Paris"} or {"ref":5,"option":"French"}, never {"ref":10,"text":"Paris","option":""}. Fields must belong to the same form/dialog. This action never clicks, submits, or navigates; commit separately after checking field_results and the returned snapshot.',
              items: {
                type: 'object',
                properties: {
                  ref: { type: 'integer', description: 'Field ref from the latest snapshot.' },
                  text: { type: 'string', description: 'Text to type into a text field. Send this key only for text fields; an empty string clears the field, so do not send it as a placeholder alongside option.' },
                  option: { type: 'string', description: 'Visible option label to select in a dropdown. Send this key only for dropdowns, never as an empty placeholder alongside text.' }
                },
                required: ['ref']
              }
            },
...

    {
      ...
      function: {
        name: 'generate_image',
        description: 'Generate an image from a text prompt using an image generation model. The image is automatically displayed inline in the chat. To iterate on a previously generated image, find its blob ID in the context (shown as __blob:N__ in the displayed_to_user field of the result of the call that generated it) and pass that N as source_blob_id, then describe only the changes in prompt. To use images already in the conversation as visual references for a NEW image (a logo to include, a product or person to match, a style to follow), pass their blob ids in reference_blob_ids (up to 3); references may be combined with source_blob_id. If source_blob_id or a reference blob is not found or is not a valid image, that image is silently skipped, and if no image resolves the tool falls back to a text-only generation using the prompt alone rather than returning an error. Returns { ok, prompt, aspect_ratio, width, height }: aspect_ratio is the ratio the image was actually generated at and width/height are its real pixel dimensions (0 when they could not be measured). Check aspect_ratio against what the user asked for before you report the image as done. When ok is true the image has already been shown to the user. Calling this tool again does NOT replace the previous image: every generated image stays visible in the chat and every call is billed, so do not regenerate to polish or second-guess a successful result. If you do generate a replacement (for example after getting the aspect ratio wrong), your reply must tell the user that you regenerated it, why, and which of the images shown is the final one.',
        parameters: {
          type: 'object',
          properties: {
            prompt: { type: 'string', description: 'The image generation prompt, or the change description when iterating. Do not name an aspect ratio, orientation, or pixel size in this text that contradicts the aspect_ratio argument; the aspect_ratio argument alone decides the geometry.' },
            source_blob_id: { type: 'integer', description: 'Optional. The blob_id (a positive integer) of a previously generated image to iterate on. Omit entirely for new images; do not pass 0, null, or an empty value. Omit it too when regenerating because the previous image had the wrong aspect ratio, since seeding from a wrong-shaped image constrains the new composition to the old one.' },
            reference_blob_ids: { type: 'array', items: { type: 'integer' }, description: 'Optional. Up to 3 blob ids of images already in the conversation to use as visual references (guides) for a NEW image, for example a logo to include, a product or a person\'s likeness to match, or a style to follow. Get these ids from the blob_id attribute on <image>, <screenshot>, and <file> placeholders in the attached context, or from a note\'s attachment list. This is distinct from source_blob_id and can be combined with it: source_blob_id seeds a prior generation to iterate on, while these steer a fresh composition. Ids past the first 3, and any id that is missing or not an image, are silently skipped; if none resolve and there is no source_blob_id, generation falls back to text-only from the prompt alone.' },
            aspect_ratio: { type: 'string', enum: ['1:1', '16:9', '9:16', '4:3', '3:4'], description: 'Required. Desired aspect ratio, and the only thing that controls the output geometry. Use "1:1" for square, "16:9" for landscape/widescreen, "9:16" for portrait/mobile, "4:3" for traditional landscape, "3:4" for traditional portrait. These five are the ONLY supported values: when the user asks for a ratio that is not listed, pick the closest listed one with the same orientation and say which you used (a portrait request such as 4:5 or 5:7 maps to "3:4", a tall mobile request maps to "9:16", a landscape request such as 3:2 maps to "4:3"). Note that "4:3" is landscape and "3:4" is portrait: check the orientation you intend against the value you pass.' }
          },
